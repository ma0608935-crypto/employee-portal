            ...
            if login_btn:
                if not username or not password:
                    ...
                    if user:
                        ...
                        # Persist to cookie so refresh keeps user logged in
                        save_fn = st.session_state.get("_save_session")
                        if save_fn:
                            save_fn(user)
                        st.rerun()
                    else:
...
def logout():
    """Clear session, cookie, and rerun."""
    clear_fn = st.session_state.get("_clear_session")
    if clear_fn:
        clear_fn()

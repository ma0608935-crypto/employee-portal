...
def _render_topbar(user):
    ...
    with c3:
        if st.button("⏻  Sign Out", key="logout_btn"):
            logout()
    st.markdown('<hr style="border-color:#2E3350;margin:0 0 4px 0">', unsafe_allow_html=True)    performance_dashboard(user)
    st.markdown('<div style="height:16px"></div>', unsafe_allow_html=True)
    st.markdown('<hr style="border-color:#2E3350;margin:0">', unsafe_allow_html=True)
    st.markdown('<div style="height:8px"></div>', unsafe_allow_html=True)
    ...
    # Use a selectbox instead of st.tabs to avoid rendering ALL tabs at once
    # (which causes duplicate key errors when the same widget appears in multiple tabs)
    selected_tab = st.radio(
        "Navigation",
        tab_labels,
        horizontal=True,
...
def _render_topbar(user):
    ...
    with c1:
        ...
        <div style="display:flex;align-items:center;gap:10px;padding:0.6rem 0">
            <span style="font-family:'Space Grotesk',sans-serif;font-size:1.3rem;
                         font-weight:700;background:linear-gradient(135deg,#4F6BFF,#7C3AED);
                         -webkit-background-clip:text;-webkit-text-fill-color:transparent">
                <img src="https://plain-eeur-prod-public.komododecks.com/202608/09/cTbwjWfVAMvKzMZ4n8ET/image.png" style="height:52px;vertical-align:middle;margin-right:10px"> Hunter Portal
            </span>
        </div>
        """, unsafe_allow_html=True)
    with c2:
    ...
    with c3:
        if st.button("⏻  Sign Out", key="logout_btn"):
            logout()
    st.markdown('<hr style="border-color:#2E3350;margin:0 0 4px 0">', unsafe_allow_html=True)        notes_panel(user)
    ...
    if is_admin:
        tabs = st.tabs([
            "👤 Profile", "💰 Sales", "📋 Attendance",
            "☕ Breaks", "📞 Callbacks", "🗺️ Maps", "🛡️ Admin"
        ])
        t_profile, t_sales, t_attend, t_breaks, t_callbacks, t_maps, t_admin = tabs
    else:
        tabs = st.tabs([
            "👤 Profile", "💰 Sales", "📋 Attendance",
            "☕ Breaks", "📞 Callbacks", "🗺️ Maps"
        ])
        t_profile, t_sales, t_attend, t_breaks, t_callbacks, t_maps = tabs
...
def _render_topbar(user):
    """Compact top navigation bar with user info and logout."""
    role_color = {"admin": "#FF6B6B", "leader": "#FFD166", "employee": "#06D6A0"}
    color = role_color.get(user["role"], "#8B90A8")
